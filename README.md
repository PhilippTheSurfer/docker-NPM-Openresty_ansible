# Nginx Proxy Manager with Openresty Ansible Repo

This repository contains Ansible roles for deploying and maintaining a Frontproxy Docker instance. The role included is:

## Deploy Role
Deploys a standard setup of Nginx-Proxy-Manager with a Openresty image and mysql. 
...
To deploy the Frontproxy, run the following command:
```bash
...
``` 
This command calls the molecule plugin to create a scenario called `default` with `docker` as the driver that shell be used to create the test environment later while executing the `molecule create` command.
